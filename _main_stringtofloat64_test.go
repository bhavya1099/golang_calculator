...
func TestStringToFloat64(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			res, err := stringToFloat64(tc.inputStr)

			if err != nil && !tc.shouldError {
				t.Errorf("Failed %s: didn't expect an error but got one: %v", tc.name, err)
				return
			} else if err == nil && tc.shouldError {
				t.Errorf("Failed %s: expected an error but didn't get one", tc.name)
				return
